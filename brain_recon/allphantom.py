...
def create_parametric_maps(data_slice, mask):
    ...
    T1[gm_mask] = 1.2  # 灰质 T1=1200ms
    T1[csf_mask] = 4.0  # CSF T1=4000ms
    T1[~mask] = 1e6  # 背景 T1=inf

    T2 = np.zeros_like(data_slice)
    T2[wm_mask] = 0.08  # 白质 T2=80ms
...
# ====================== 批处理 ======================
def batch_process_ixi_t2(input_dir, output_dir):
    # 确保输出目录存在
    ...
    for idx, filename in enumerate(os.listdir(input_dir)):
        if filename.endswith(".nii.gz"):
            ...
            output_phantom = os.path.join(output_dir, f"EpiSE_70_256_{idx + 1}.phantom")
            print(f"Processing {filename} -> {output_phantom}")
            nii_to_phantom(input_path, output_phantom)


# ====================== 执行 ======================
if __name__ == "__main__":
    input_dir = "nii_path"
    output_dir = "phantom_path"
    batch_process_ixi_t2(input_dir, output_dir)